...
const getLocName = () => {
  ...
  selector.addEventListener("change", () => {
    ...
    document.querySelector(
      "#elevation"
    ).innerText = `Elevation: ${elevation} ft.`;
...
const displayWeather = (data) => {
  ...
  //const { accumulation } = data.current.snow.1h;
  //const { 1h } = data.current.snow;
  console.log(description);

  //const { description } = data.weather;
  //document.querySelector("#current-cond").innerText = "Current Conditions";

  document.querySelector("#current-temp").innerText =
    "Currently " + Math.round(temp) + " F";
  document.querySelector("#current-wind").innerText =
    "Wind: " +
    Math.round(wind_speed) +
    " mph, gusting to " +
    Math.round(wind_gust) +
    " mph";
...
const updateImage = (locName) => {
  ...
  document.querySelector("#firstmain").src = `./images/${selPhoto}`;
};
